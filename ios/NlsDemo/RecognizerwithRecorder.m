...
#import "RecognizerwithRecorder.h"

//一句话识别-简易版
@interface RecognizerwithRecorder()<NlsSpeechRecognizerwithRecorderDelegate>{
    IBOutlet UITextView *textViewRecognizerwithRecorder;
    IBOutlet UISwitch *switchRecognizerwithRecorder;
    Boolean recognizerStarted;
}
@end

@implementation RecognizerwithRecorder
...
- (IBAction)startRecognizerwithRecorder {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        // UI更新代码
        [self->switchRecognizerwithRecorder setOn:true];
        self->textViewRecognizerwithRecorder.text = @"start Recognize！";
    });
}
...
-(void)OnRecognizedCompleted:(NlsDelegateEvent)event result:(NSString *)result statusCode:(NSString*)statusCode errorMessage:(NSString*)eMsg{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        // UI更新代码
        self->textViewRecognizerwithRecorder.text = result;
        NSLog(@"%@", result);
        [self->switchRecognizerwithRecorder setOn:false];
    });
}
